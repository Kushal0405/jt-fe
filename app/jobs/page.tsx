...
import { Search, Plus, MapPin, Briefcase, Clock, ExternalLink, BookmarkPlus, SlidersHorizontal } from 'lucide-react';
...
export default function JobsPage() {
  ...
  async function saveJob(job: Job) {
    setSaving(job._id);
    try {
      await applicationsApi.create({ jobId: job._id, stage: 'saved' });
    ...
    <AppLayout>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {jobs.map(job => (
            <JobCard key={job._id} job={job} onSave={() => saveJob(job)} saving={saving === job._id} />
          ))}
        </div>
      )}
...
function JobCard({ job, onSave, saving }: { job: Job; onSave: () => void; saving: boolean }) {
  return (
    <div className="card gradient-border p-5 hover:glow transition-all duration-300 flex flex-col gap-4">
      {/* Header */}
